...
npx tsx playground/content_enhancer_agent/index.ts -i "Lui ha detto che veniva pero poi non si è fatto vivo."
npx tsx playground/content_enhancer_agent/index.ts -i "La riunione è andata bene ma cerano alcune cose che non erano chiare."
...
npx tsx playground/content_enhancer_agent/index.ts -i "Il nostro team ha lavorato tanto su questo progetto e adesso siamo contenti perche i risultati sono buoni." -l moderate
npx tsx playground/content_enhancer_agent/index.ts -i "La citta è molto bella pero il traffico è un problema grande per tutti quelli che ci vivono." -l moderate
...
npx tsx playground/content_enhancer_agent/index.ts -i "L'intelligenza artificiale sta cambiando tutto e ci sono tante applicazioni che prima non esistevano e che adesso sono molto utili per le aziende e per le persone normali." -l heavy
npx tsx playground/content_enhancer_agent/index.ts -i "Il sistema sanitario italiano ha bisogno di essere migliorato perche ci sono tanti problemi con le liste di attesa e la qualita dei servizi non è sempre buona." -l heavy
```
